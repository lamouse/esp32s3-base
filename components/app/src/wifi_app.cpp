...
static void mask_event_cb(lv_event_t *e) {
  ...
  if (code == LV_EVENT_COVER_CHECK) {
    ...
    lv_draw_mask_fade_param_t *fade_mask_top =
        static_cast<lv_draw_mask_fade_param_t *>(
...
static void list_btn_cb(lv_event_t *e) {
  ...
  lv_obj_set_style_shadow_opa(btn_back, LV_OPA_TRANSP,
                              LV_PART_MAIN); // 阴影透明
  lv_obj_add_event_cb(btn_back, btn_back_cb, LV_EVENT_ALL,
                      NULL); // 添加按键处理函数
  ...
  lv_textarea_set_password_mode(ta_pass_text,
                                false); // 是否使用密码输入显示模式
  ...
  lv_obj_add_event_cb(btn_connect, btn_connect_cb, LV_EVENT_ALL,
                      NULL); // 事件处理函数
  ...
  lv_obj_add_event_cb(roller_num, mask_event_cb, LV_EVENT_ALL,
                      NULL); // 事件处理函数
  ...
  lv_obj_add_event_cb(btn_num_ok, btn_num_cb, LV_EVENT_ALL,
                      NULL); // 事件处理函数
  ...
  lv_obj_set_style_bg_opa(roller_letter_low, LV_OPA_50,
                          LV_PART_SELECTED); // 设置选中项的透明度
  lv_roller_set_options(roller_letter_low, opts_letter_low,
  ...
  lv_obj_add_event_cb(roller_letter_low, mask_event_cb, LV_EVENT_ALL,
                      NULL); // 事件处理函数
  ...
  lv_obj_add_event_cb(btn_letter_low_ok, btn_letter_low_cb, LV_EVENT_ALL,
                      NULL); // 事件处理函数
  ...
  lv_obj_set_style_bg_opa(roller_letter_up, LV_OPA_50,
                          LV_PART_SELECTED); // 设置选中项的透明度
  lv_roller_set_options(roller_letter_up, opts_letter_up,
                        LV_ROLLER_MODE_INFINITE); // 循环滚动模式
  ...
  lv_obj_add_event_cb(roller_letter_up, mask_event_cb, LV_EVENT_ALL,
                      NULL); // 事件处理函数
  ...
  lv_obj_add_event_cb(btn_letter_up_ok, btn_letter_up_cb, LV_EVENT_ALL,
                      NULL); // 事件处理函数
...
static void wifi_connect(void *arg) {
  ...
  while (true) {
    ...
    if (xQueueReceive(xQueueWifiAccount, &wifi_account, portMAX_DELAY)) {
      device::wifi wifi;
      wifi.connect(wifi_account.wifi_ssid, wifi_account.wifi_password,
                   [](auto event) {
...
void app_wifi_connect(void) {
  ...
  lv_obj_set_scrollbar_mode(wifi_list,
                            LV_SCROLLBAR_MODE_OFF); // 隐藏wifi_list滚动条
  // 显示wifi信息
  lv_obj_t *btn;
  for (auto &ap : ap_list) {
  ...
  xTaskCreatePinnedToCore(wifi_connect, "wifi_connect", 4 * 1024, NULL, 5, NULL,
                          1); // 创建wifi连接任务
}
} // namespace app